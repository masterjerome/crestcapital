  ...
  return (
    <>
      {requestATM && (
        <ATMRequestForm
          hideForm={hideForm}
          hasPreviouslyRequested={hasRequested}
          email={email}
          name={name}
        />
      )}
      <main className="border border-[#7676801F] rounded-xl px-3 md:px-4 xl:px-6 py-6 mb-24 xl:mb-4">
        <p className="text-sm xl:text-base text-[#06121B] font-semibold">
          Cards
        </p>
        <div className="overflow-x-auto special pb-4 mt-6 flex gap-x-5">
          {hasRequested && (
            <div
              style={{
                background:
                  "linear-gradient(31.58deg, #592F1A -2.49%, #191C2E 67.92%)",
              }}
              className="p-6 min-w-[252px] w-[252px] h-40 rounded-2xl cursor-pointer"
              onClick={() =>
                toast.info("Your Debit card details is currently unavailable.")
              }
            >
          ...
          <div className="min-w-[252px] w-[252px] h-40 bg-[#EBEBF52E] rounded-2xl flex flex-col items-center justify-center border border-[#E8E9EB]">
            <div
              onClick={hideForm}
              className="min-w-[3rem] min-h-[3rem] size-8 md:size-10 lg:size-12 bg-[#B9BAC0] flex items-center justify-center rounded-[50%] cursor-pointer hover:bg-secondary duration-500"
            >
              <Add size="16" color="#FFF" />
            </div>
            <p className="text-[8px] md:text-[10px] xl:text-[12px] font-medium text-secondary">
              Add a card
            </p>
          </div>
        </div>
